...
export default function InsideOutHome4() {
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-blue-900/95 backdrop-blur-sm border-b border-white/10">
        <nav className="container mx-auto px-4 py-4">
          ...
          {mobileMenuOpen && (
            <div className="md:hidden bg-blue-900/98 backdrop-blur-sm border-t border-white/10 py-4 mt-4">
              <div className="flex flex-col space-y-4">
                <button 
                  onClick={() => { document.getElementById('why-choose')?.scrollIntoView({ behavior: 'smooth' }); setMobileMenuOpen(false); }}
                  className="text-white hover:text-blue-200 transition-colors text-sm font-medium text-left py-2"
                >
                  Why Choose Us
                </button>
                <button 
                  onClick={() => { document.getElementById('our-process')?.scrollIntoView({ behavior: 'smooth' }); setMobileMenuOpen(false); }}
                  className="text-white hover:text-blue-200 transition-colors text-sm font-medium text-left py-2"
                >
                  Our Process
                </button>
                <button 
                  onClick={() => { document.getElementById('global-talent')?.scrollIntoView({ behavior: 'smooth' }); setMobileMenuOpen(false); }}
                  className="text-white hover:text-blue-200 transition-colors text-sm font-medium text-left py-2"
                >
                  Global Talent
                </button>
                <button 
                  onClick={() => { document.getElementById('roles')?.scrollIntoView({ behavior: 'smooth' }); setMobileMenuOpen(false); }}
                  className="text-white hover:text-blue-200 transition-colors text-sm font-medium text-left py-2"
                >
                  Roles
                </button>
        ...
        <div className="container relative z-10" style={{ paddingTop: "24px", paddingBottom: "24px" }}>
          ...
          <div className="text-center text-white space-y-4 mb-8">
            ...
            <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold leading-tight max-w-4xl mx-auto px-4">
              South African VAs: Native English, College Educated, 90% Lower Cost
            </h1>
            
            <p className="text-sm sm:text-base md:text-lg text-blue-100 leading-relaxed max-w-2xl mx-auto px-4">
              Get a pre-vetted, full-time professional who speaks English as a first language and treats your business like the opportunity of a lifetime. Done for you recruitment — start in days, not weeks.
            </p>
          </div>
